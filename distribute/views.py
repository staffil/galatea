...
def distribute_app(request, llm_id):
    llm = get_object_or_404(LLM, id=llm_id, user=request.user)
    genre_list = Genre.objects.all()
    ...
    if request.method == 'POST':
        ...
        if not distribute_text or not title:
            messages.error(request, _("AI 설명을 입력해주세요."))
            return redirect('distribute:distribute_app', llm_id=llm.id)
        elif len(title) > 50:
            messages.error(request, _("AI 제목은 50자 이내입니다."))
            return redirect('distribute:distribute_app', llm_id=llm.id)
        elif len(selected_genres) > 5:
            messages.error(request, _("장르는 5개까지 선택 가능합니다."))
            return redirect('distribute:distribute_app', llm_id=llm.id)
    ...
    return render(request, "distribute/app/distribute_app.html", context)
